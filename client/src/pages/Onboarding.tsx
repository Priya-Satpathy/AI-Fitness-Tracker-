...
const Onboarding = () => {
  ...
  const handleNext = async ()=>{
      ...
      if(step < totalSteps){
        ...
        await mockApi.user.update(user?.id || "", userData as unknown as 
          Partial <UserData> )
          ...
          <div className='flex items-center gap-3 mb-2'>
            <div className='w-10 h-10 rounded-xl bg-emerald-500
             flex items-center justify-center'>
              <PersonStanding className="w-6 h-6 white" />
            </div>
            <h1 className='text-2xl font-bold text-slate-800
             dark:text-white'>Fit Track</h1>
          </div>
            ...
            <div className='flex gap-2 max-w-2xl'>
                  {[1,2,3].map((s)=>(
                      <div key={s} className={`h-1.5 flex-1 rounded-full
                        transition-all duration-300 ${s <= step ? 
            ...
            <div className='space-y-6'>
                <div className='flex items-center gap-4 mb-8'>
                  ...
                  <div>
                    ...
                    text-slate-800 dark: text-white'>How old are you?
                    </h2>
                    <p className='text-slate-500
                    dark:text-slate-400'>This help us calculate your needs</p>
                  </div>
                </div>
                <Input label='Age' type = 'number' 
                className='max-w-2xl' value={formData.age} onChange=
                {(v)=>updateField('age',v)} placeholder='Enter your age' min = {13} max = {120} required/>
            </div>
          )}
           ...
           {step === 2 && (
            <div className='space-y-6 onboarding wrapper'>
                <div className='flex items-center gap-4 mb-8'>
                  ...
                  <div>
                    <h2 className='text-lg font-semibold
                    text-slate-800 dark: text-white'>Your measurements
                    </h2>
                    <p className='text-slate-500
                    dark:text-slate-400 text-sm'>Help us to track your progress</p>
                  </div>
                </div>

                <div className='flex flex-col gap-4 max-w-2xl'>
                  <Input label='Weight (kg)' type = 'number' 
                  value={formData.weight} onChange=
                  {(v)=>updateField('weight',v)} placeholder='Enter your weight'
                   min = {20} max = {300} required/>

                   <Input label='Height (cm) - Optional' type = 'number' 
                  value={formData.height} onChange=
                  {(v)=>updateField('height',v)} placeholder='Enter your height'
                   min = {100} max = {250} />
                </div>
           ...
           {step === 3 && (
            <div className='space-y-6 onboarding wrapper'>
                <div className='flex items-center gap-4 mb-8'>
                  ...
                  <div>
                    <h2 className='text-lg font-semibold
                    text-slate-800 dark: text-white'>What's your
                    goal?
                    </h2>
                    <p className='text-slate-500
                    dark:text-slate-400 text-sm'>We'll tailor your experience</p>
                  </div>
                </div>
               ...
               <div className='space-y-8 max-w-lg'>
                      ...
                      <div className='space-y-6'>
                        <Slider label = "Daily Calorie Intake" min = {120} max = {4000} step =
                        {50} value = {formData.dailyCalorieIntake} onChange={(v)=>updateField ('dailyCalorieIntake',v)} unit = "kcal" infoText = "The total calories you plan to consume each day."/>
